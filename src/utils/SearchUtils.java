...
public class SearchUtils {
	...
	public static void getIds(String input, OnLoadSearchFinishListener listener) {
		...
		String key = deCondeKey(input);// 解析用户输入关键字为UTF-8
		...
		for (int i = 0; i < idSize; i++) {
			...
			try {
				...
				for (Element e : element) {
					...
					music.setTitle(getSubString(e.select("title").text()));
					music.setArtist(e.select("artist").text());
					...
					// 对加密过后的歌曲在线地址进行解密
					music.setUrl(StringUtils.decodeMusicUrl(e.select(
							"location").text()));
